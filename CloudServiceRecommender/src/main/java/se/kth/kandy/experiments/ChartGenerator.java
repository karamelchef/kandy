...
/**
 * on Linux jpeg file is created under
 * /payara41/glassfish/domains/domain1/config
 *
 * @author Hossein
 */
@Stateless
public class ChartGenerator {
    ...
    public void jpgXYChartProfitErrorVsTrueCost(ArrayList<InstanceZoneProfit> izProfits) {

        final XYSeries estimatedProfit = new XYSeries("estimated profit");
        final XYSeries trueCost = new XYSeries("true cost");

        for (int i = 0; i < izProfits.size(); i++) {
            estimatedProfit.add(i,izProfits.get(i).getEstimatedProfit());
           trueCost.add(i,izProfits.get(i).getTrueCost());
        }
        final XYSeriesCollection dataset = new XYSeriesCollection();
        dataset.addSeries(estimatedProfit);
        dataset.addSeries(trueCost);

        JFreeChart lineChartObject = ChartFactory.createXYLineChart(
                "Estimated profit vs True cost",
                "Sample instance number", "Estimated profit and true cost $",
        ...
        String fileName = "estimatedProfitVsTrueCost.jpeg";
